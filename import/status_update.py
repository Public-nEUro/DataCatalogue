...
def _normalise_replacement(value):
    value = value.strip()
    if re.fullmatch(r"10\.\d{4,9}/\S+", value, flags=re.I):
        value = "https://doi.org/" + value
    elif re.match(r"^(?:https?://)?(?:dx\.)?doi\.org/", value, flags=re.I):
        value = re.sub(
            r"^(?:https?://)?(?:dx\.)?doi\.org/",
            "https://doi.org/",
            value,
    ...
    if parsed.scheme in {"http", "https"} and parsed.netloc:
        return value
    raise ValueError("--new must be a DOI or an http(s) URL")
...
def update_status(path, status, contact=None, reason=None, new=None):
    ...
    if reason:
        record["status_note"] = reason.strip()
    if new:
        record["replacement"] = _normalise_replacement(new)
...
def main():
    ...
    parser.add_argument("--reason", help="Status note explaining why data were withdrawn")
    parser.add_argument("--new", help="Replacement DOI or http(s) URL for superseded data")
